if(spd > 0)
{
	...
	if(see_check)
	{
		if(((_wallsee and _doorsee) and abs(angle_difference(weapon.image_angle,pd1)) < 5) or (check == true and !(_wallsee2 and _doorsee2)) or (_wt and _dt) or (_wallsee2 and _doorsee2))
		{
			...
			suq_loc = true;
			if(((_wallsee and _doorsee) or (_wallseed and _doorseed) or (_wt and _dt) or (_wallsee2 and _doorsee2)) and spd > 0)
			{ shootCheck -= 1; }
			...
			if(shootCheck <= 0)
			{
				...
				{ 
					...
					else if(bulamt > 25) { bulamt = 0; shtspd = 80; instance_create_layer(x,y,"Instances_Action",obj_box); }
				}
	...
	if(drive)
	{
		...
		y = o_badCar.y;
		o_badCar.speed = 6;
		o_badCar.direction = image_angle;
	}
