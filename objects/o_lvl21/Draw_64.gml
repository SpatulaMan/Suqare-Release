...
if(start == true and (m > 0 or s > 0 or ml > 0))
{
	...
	if(s1) { draw_text_transformed_colour(obj_suq.roomw*.01,obj_suq.roomh*.2,string("Speed Throwing Points: " + string(spd)),2,2,0,c_white,c_white,c_white,c_white,1); }
...
if(m <= 0 and s <= 0 and ml <= 0 and start == true)
{
	start = false;
	a = false;
	s = false;
