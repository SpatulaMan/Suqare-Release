...
if(soundCheck == true)
{
	...
	y3 = y2;
	suq_loc = false;
	if(_doorsee and _wallsee) { shootCheck -= 1; }
...
if((see_check and abs(angle_difference(weapon.image_angle,pd1)) < 10) or check == true or (_wt and _dt))
{
	...
	if(path_check)
	{
		...
		else if(mp_grid_path(global.grid,path,x,y,obj_suq.x,obj_suq.y,true) and hit == false and suq_loc == true)
		{
			path_start(path,1,path_action_stop,false);
			alarm_set(0,0);
			alarm_set(1,0);
...
if(hp < hpc)
{
	check = true;//testing to see if this works
	hpc = hp;
