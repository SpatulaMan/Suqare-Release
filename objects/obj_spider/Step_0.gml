...
if(!path_exists(path))
{
	...
	if(((_wallsee and _doorsee) or (_wt and _dt)))
	{
		...
		if(_wt and _dt and !_wallsee and !_doorsee and instance_exists(obj_turret))
		{
			...
			y3 = _tn.y;
			suq_loc = false;
			image_angle = point_direction(x,y,_tn.x+x4,_tn.y+y4);
		}
		else
		{
			image_angle = point_direction(x,y,obj_suq.x+x4,obj_suq.y+y4);
			x3 = obj_suq.x;
			y3 = obj_suq.y;
			suq_loc = true;
		}
		...
		if(instance_exists(_tn))
		{
			if(place_meeting(x,y,_tn) or distance_to_object(_tn) < 3)
			{
				shootCheck -= 1; 
			}
			else { shootCheck = 40; }
	...
	if(hp <= 0)
	{
		audio_play_sound(snd_enemyExplode,3,false,random_range(0.01,0.08)*o_saveload.sfxvol,0,random_range(1.7,2.2));
		var _piece = instance_create_layer(x,y,"Instances_Action",o_pieces);
		_piece.sprite_index = s_spider_1;
		_piece.speed = 5;
		...
		_piece1.sprite_index = s_spider_1;
		_piece1.speed = 5;
		...
		_piece2.sprite_index = s_spider_1;
		_piece2.speed = 5;
