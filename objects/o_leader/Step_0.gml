...
if(access == false and _wallsee and _doorsee and hitbool and abs(angle_difference(weapon.image_angle,pd)) < 5)
{
	see_check = true;
}
else if(_wallsee and _doorsee and hitbool and
...
if(see_check)
{
	if(((_wallsee and _doorsee) and hitbool and abs(angle_difference(weapon.image_angle,pd1)) < 5) or (check == true and !(_wallsee2 and _doorsee2)) or (_wt and _dt) or (_wallsee2 and _doorsee2))
	{
		check = true;
		x3 = obj_suq.x;
		y3 = obj_suq.y;
		...
		else if(_wt and _dt and _wallsee and _doorsee and hitbool and instance_exists(obj_turret))
		{
		...
		suq_loc = true;
		if(((_wallsee and _doorsee) or (_wallseed and _doorseed) or (_wt and _dt) or (_wallsee2 and _doorsee2)) and hitbool) { shootCheck -= 1; }
	...
	if(path_check and room != r_lvl_19 and room != r_lvl_29 and room != r_lvl_41)
	{
