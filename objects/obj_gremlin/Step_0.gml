...
if((boss == true and instance_exists(o_wallCreate.inst)) or boss == false)
{
	...
	if(((_wallsee and _doorsee) or (_wt and _dt)))
	{
		...
		{ 
			...
			audio_play_sound(snd_fire,4,false,random_range(0.9,0.98)*o_saveload.sfxvol,0,random_range(0.2,0.4)); 
		}
		check = true;
		...
		if(place_meeting(x,y,obj_suq) or distance_to_object(obj_suq) < 3) { shootCheck -= 1; }
		else { shootCheck = 15; }
		if(instance_exists(obj_turret))
		{
			if(place_meeting(x,y,_tn))
			{
				shootCheck -= 1; 
			}
			else { shootCheck = 15; }
	...
	if(path_check)
	{
		...
		else if(mp_grid_path(global.grid,path,x,y,obj_suq.x + x4,obj_suq.y + x4,true) and hit == false and suq_loc == true)
		{
			path_start(path,path_speed,path_action_stop,false);
			alarm_set(0,0);
			alarm_set(1,0);
	...
	if(hp <= 0)
	{
		...
		_piece1.sprite_index = s_enemy_G_1;
		_piece1.speed = 5;
		...
		_piece2.sprite_index = s_enemy_G_1;
		_piece2.speed = 5;
